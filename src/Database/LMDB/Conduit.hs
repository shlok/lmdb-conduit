...
import Database.LMDB.Conduit.Internal (marshalIn, marshalOut, noWriteFlags)
...
yieldAll :: (MonadIO m) => MDB_cursor' -> Ptr MDB_val -> Ptr MDB_val -> Bool -> ConduitT i (ByteString, ByteString) m ()
...
sinkLMDB env dbi = do
    ...
    awaitForever $ \(k, v) -> liftIO . runEmpty channel . marshalOut k $ \k' -> marshalOut v $ \v' ->
