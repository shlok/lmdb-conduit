...
import qualified Database.LMDB.Conduit.Tests (tests)
...
main = do
    ...
    defaultMain $ withResource
        (do tmpParent <- getCanonicalTemporaryDirectory
            tmpDir <- createTempDirectory tmpParent "lmdb-conduit-tests"
            env <- mdb_env_create
...
tests res =
    testGroup "Tests"
        [ testGroup "Database.LMDB.Conduit.Tests" $
                     Database.LMDB.Conduit.Tests.tests res ]
